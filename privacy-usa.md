...
**Phone number validation.** As part of fraud screening for customers in the United States, we may also use third-party phone-number validation services to check whether the telephone number you give us is valid and to identify its line type and network operator. For this purpose we send the provider your telephone number only  -  no name, order or payment details  -  and the provider uses it solely to answer our request and does not retain it. What comes back (for example whether the number is a mobile, landline or internet-based line, and the operator) is added to the fraud screening records described below. This check is not applied to customers in the United Kingdom.
...
**Phone-number validation providers.** For customers in the United States, we may disclose your telephone number to a third-party phone-number validation service so that it can be checked as described in section 4 (Phone number validation). Each such provider acts as a **service provider** under the CCPA/CPRA: it receives the telephone number only, uses it solely to answer our request, does not retain it, and may not use it for its own purposes. These providers are based in the United States, and this disclosure is made only for fraud prevention and security, not for marketing. Telephone numbers of customers in the United Kingdom are not disclosed to these providers.
...
For the same fraud-prevention purpose, we may also disclose your telephone number  -  and only your telephone number  -  to a third-party phone-number validation service, which checks whether the number is valid and identifies its line type and network operator, uses it solely to answer our request, and does not retain it (see section 4 (Phone number validation) of the main Privacy Notice).
...
We also disclose telephone numbers to third-party phone-number validation services for fraud prevention, each of which acts as a **service provider**, receives the telephone number only, uses it solely to answer our request and does not retain it (see section 7 (Phone-number validation providers) of the main Privacy Notice).
